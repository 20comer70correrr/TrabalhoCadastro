...
function listarAlunos() {
  ...
  for (var i = 0; i < listaAlunos.length; i++) {
    ...
    anchorInformacoes.setAttribute('onclick', 'InformacaoAlunos()');
    anchorInformacoes.setAttribute("id", i);
    botaoAlterar.name = i;
    anchorInformacoes.innerText = "Informações";
    botaoAlterar.onclick = pegarInformacao;
...
function pegarInformacao(evento) {
  const indice = evento.target.name;
  ...
  infoNome.name = indice;
  const aluno = listaAlunos[indice];
  infoNome.value = aluno.nome;
  infoNome.focus();
  infoCpf.value = aluno.cpf;
  infoData.value = aluno.dataNascimento;
}